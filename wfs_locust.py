...
class EcWfsTester(TaskSet):
    ...
    #     response = self._doWMS("image/png8")
    #     if "image/png" not in response.headers['Content-Type']:
    #         response.failure("Expected content image/png but found"+response.headers['Content-Type'])
    #     response.success()
    #
    # @task(1)
    # def wms_jpeg_bbox(self):
    #     response = self._doWMS("image/jpeg")
    #     if "image/jpeg" not in response.headers['Content-Type']:
    #         response.failure("Expected content image/jpeg but found"+response.headers['Content-Type'])
    #     response.success()

    # @task(1)
    # def wms_tiff_bbox(self):
    #     response = self._doWMS("image/tiff")
    #     if "image/jpeg" not in response.headers['Content-Type']:
    #         response.failure("Expected content image/tiff but found"+response.headers['Content-Type'])
    #     response.success()
